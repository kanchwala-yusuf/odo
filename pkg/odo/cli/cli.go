...
func NewCmdOdo(name, fullName string) *cobra.Command {
	...
	// Step 2: Expose commands in experimental mode, if experimental mode is enabled.
...
func addExperimentalCommands(rootCmd *cobra.Command, fullName string) *cobra.Command {
	...
	//		- setting odo preference by using the "odo preference set experimental true" command
	...
	// Fetch config and check if experimental mode has been set in odo preference.yaml
	...
	// If experimental mode set in preference or in env then add experimental commands to the rootCmd
